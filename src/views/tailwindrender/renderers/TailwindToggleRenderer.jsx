...
const TailwindToggleRenderer = React.memo((props) => {
  const { data, uischema, path, handleChange, config, visible } = props;
      ...
      {visible && (
        <div className="relative inline-block w-8 mx-2 align-middle select-none transition duration-[300ms] ease-in">
          <input
            ...
            name={path}
            id={`${path}-toggle`}
            className="toggle-checkbox absolute block w-4 h-4 -mt-0.5 rounded-full bg-white text-color-0500 ring-color-0500 border appearance-none cursor-pointer"
