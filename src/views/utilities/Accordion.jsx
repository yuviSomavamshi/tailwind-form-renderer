...
const Accordion = ({ visible = true, title, defaultOpen = false, disableOnMouseHover = false, onDelete, onChange, children }) => {
  ...
  return (
    <>
      {visible && (
        <div className="container flex flex-col mt-0.5">
          <div
            ...
            className={`container flex px-1.5 py-0.5 items-center justify-between rounded border-b-2 border-slate-300 bg-color-0100 hover:bg-color-0100 ${
              show && "bg-color-0200"
          ...
          >
            ...
            <div className="flex flex-row">
              ...
              <IconRenderer icon="ChevronRight" className={`${show ? "-rotate-90 transform" : "rotate-90"} h-5 w-5 text-color-0500`} />
            </div>
          </div>
          <Transition
            show={show}
          ...
          >
            <div className="container items-center p-1 rounded-b bg-white">{children}</div>
          </Transition>
        </div>
      )}
    </>
